...
ships_list = [] #список кораблей игрока
...
canvas.create_rectangle(0, 0, size_canvas_x, size_canvas_y, fill="aqua")
canvas.create_rectangle(size_canvas_x + menu_x, 0, size_canvas_x + menu_x + size_canvas_x, size_canvas_y, fill="teal")
canvas.pack()
tk.update()


def draw_table(offset_x=0): #отрисовываем сетку на экране
...
t0 = Label(tk, text="Первый игрок", font=("Helvetica", 16))
t0.place(x=size_canvas_x // 2 - t0.winfo_reqwidth() // 2, y=size_canvas_y+3)
t1 = Label(tk, text="Второй игрок", font=("Helvetica", 16))
t1.place(x=size_canvas_x + menu_x + size_canvas_x // 2 - t1.winfo_reqwidth() // 2, y=size_canvas_y+3)

t0.configure(bg="red")
t0.configure(bg="#f0f0f0")

#запись хода, какой игрок ходит
t3 = Label(tk, text="@@@@@@@", font=("Helvetica", 14))
t3.place(x=size_canvas_x + 10, y= 2 * step_y)

def mark_igrok(igrok_mark_1):
    if igrok_mark_1:
        t0.configure(bg="red")
        t1.configure(bg="#f0f0f0")
        t3.configure(text="Ход Второго Игрока")
    else:
        t1.configure(bg="red")
        t0.configure(bg="#f0f0f0")
        t3.configure(text="Ход Первого Игрока")
mark_igrok(hod_igrovomu_polu_1)



def button_show_enemy1(): #отображаем на экране корабли 1 игрока
...
def button_show_enemy2(): #отображаем на экране корабли 2 игрока
    for i in range(0, s_x):
        for j in range(0, s_y):
...
def button_begin_again(): #при наж на кнопку лист чистится после того как с поля удалятся все корабли
    global list_ids
    global points1
    global points2
    global boom
...
b0.place(x=size_canvas_x + 20, y=10)

b1 = Button(tk, text="Начать заново!", command=button_begin_again)
b1.place(x=size_canvas_x + 50, y=40)
...
def draw_point(x, y):
    ...
    if enemy_ships1[y][x] == 0:
        ...
        id2 = canvas.create_oval(x * step_x + step_x // 3, y * step_y + step_y // 3, x * step_x + step_x - step_x // 3,
                                 y * step_y + step_y - step_y // 3, fill="aqua")
        list_ids.append(id1)
        list_ids.append(id2)
    if enemy_ships1[y][x] > 0:
        color = "purple"
...
def draw_point2(x, y, offset_x=size_canvas_x + menu_x):
    ...
    if enemy_ships2[y][x] == 0:
        ...
        id2 = canvas.create_oval(offset_x + x * step_x + step_x // 3, y * step_y + step_y // 3, offset_x + x * step_x + step_x - step_x // 3,
                                 y * step_y + step_y - step_y // 3, fill="teal")
        list_ids.append(id1)
        list_ids.append(id2)
    if enemy_ships2[y][x] > 0:
        color = "purple"
...
def add_to_all(event): #при клике определяем координаты ячейки
    ...
    # клик находится в пределах 1 игровой области
        ...
        if points1[ip_y][ip_x] == -1:
            ...
            if check_winner2():
                ...
                id2 = canvas.create_rectangle(step_x * 3 + step_x // 2, step_y * 3 + step_y // 2,
                                              ...
                                              size_canvas_y - step_y - step_y // 2, fill="light blue")
                list_ids.append(id2)
                id3 = canvas.create_text(step_x * 12, step_y * 5, text=winner, font=("Arial", 40), justify=CENTER)
                id4 = canvas.create_text(step_x * 12, step_y * 7, text=winner_add, font=("Arial", 30), justify=CENTER)
                list_ids.append(id3)
                list_ids.append(id4)

        #print(len(list_ids))

    # клик находится в пределах 2 игровой области
...
canvas.bind_all("<Button-1>", add_to_all)  # ЛКМ  привязка событий при нажатие
...
def generate_enemy_ships(): #генерируем корабли
    global ships_list
    enemy_ships =[]
